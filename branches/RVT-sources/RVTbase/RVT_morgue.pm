...
sub RVT_mount_list {
   ...
   print "\nMounted CIFS: \n";
   open ( MOUNT, 'mount | grep " type cifs " |' ) or RVT_log('CRIT', "couldn't execute mount: $!");
   while (my $l=<MOUNT>) {
   	next unless ( $l=~/\/\d{6}-\w+\/\d{6}-\d\d-\d\d?\/mnt/ );
   	$l =~  /^([^ ]+ on [^ ]+) type cifs/ ;
	print "\t$1\n";
   }
   close MOUNT;
   ...
   print "\nMounted SSH: \n";
   open ( MOUNT, 'mount | grep " type ssh " |' ) or RVT_log('CRIT', "couldn't execute mount: $!");
   while (my $l=<MOUNT>) {
   	next unless ( $l=~/\/\d{6}-\w+\/\d{6}-\d\d-\d\d?\/mnt/ );
   	$l =~  /^([^ ]+ on [^ ]+) type ssh/ ;
	print "\t$1\n";
...
sub RVT_losetup_recheck {
  ...
  if ($object eq 'all') {
  	  $all = 1;
	  $cases = $main::RVT_cases->{case};  
  } else {
	  RVT_fill_level(\$object);
	  $acase = RVT_chop_diskname ('case', $object);
	  if ( RVT_check_format($acase) ne 'case number' ) {
			RVT_log ('ERR', 'RVT_losetup_recheck expected a case');
			return 0;
	  }
	  $cases = { $acase  =>  $main::RVT_cases->{case} };
  }
...
sub RVT_mount_delete () {
	...
	my @objs;
	push (@objs, 
		RVT_exploit_diskname ('partition', $object), 
		RVT_exploit_diskname ('network',   $object)
		);
	return 0 unless (@objs);
	
	foreach my $o ($objs) {
		    my ($case, $device, $disk, $part, $net);
			my $r = RVT_split_diskname($o);
			$case =   $r->{case};
			$device = $r->{device};
			$disk =   $r->{disk};
			$part =   $r->{partition};	
			...
			my $ppart; 
			$ppart = "$pmnt/p$part" if ($part);
			$ppart = "$pmnt/n-$net" if ($net);
			if (!$ppart) {
				RVT_log('ERR', "Couldn't umount $case-$device-$disk-" . (($part)?"p$part":"n-$net") );
				next;
			}
...
#TODO
#HECHO 1 corregir mount_assign:  exploit_diskname: que soporte también net 
#HECHO 2 corregir mount_delete igual.
#2.5 comprobar que esto tira
#3 y f-response??
...
sub RVT_mount_assign () {
	...
	my @parts = RVT_exploit_diskname ('partition', $object);
	my @nets  = RVT_exploit_diskname ('network',   $object);
	return 0 unless (@parts or @nets);
	...
	foreach my $p (@parts) {
		...
		if (! -d $ppart ) { mkpath ($ppart) or RVT_log('CRIT' , "couldn't create directory $!"); }
	...
	foreach my $n ( @nets ) {
		my $r = RVT_split_diskname($n);
		$case = $r->{case};
		$device = $r->{device};
		$disk = $r->{disk};
		$net = $r->{network};
		
		my ($type, $resource, $user, $password);
		$type = 	$main::RVT_cases->{case}{$case}{device}{$device}{disk}{$disk}{network}{$n}{type};
		$resource = $main::RVT_cases->{case}{$case}{device}{$device}{disk}{$disk}{network}{$n}{resource};
		$user = 	$main::RVT_cases->{case}{$case}{device}{$device}{disk}{$disk}{network}{$n}{user};
		$password = $main::RVT_cases->{case}{$case}{device}{$device}{disk}{$disk}{network}{$n}{password};
		
		# sshfs requires an echo an a pipe in order to get the password,
		# so 'system' cannot be used with arguments (with the security risk that implies)
		# TODO Arguments MUST be cleaned!!
		
		my $ppart = "$pmnt/n-$n";
		if (! -d $ppart ) { mkpath ($ppart) or RVT_log('CRIT' , "couldn't create directory $!"); }
		
		my $connstr;	
		if ( $type eq 'cifs' || $type eq 'CIFS' ) {
			$connstr = "mount -t '$resource' $ppart -o user=$user,password=$password,ro,iocharset=utf8";
		} elsif ( $type eq 'ssh' || $type eq 'SSH' ) {
			$connstr = "echo '$password' | sshfs " . $user ."@" . $resource." $ppart -o password_stdin";
		} else {
			RVT_log ('ERR', 'unknown network source type');
			return 0;
		}
		
		#TODO jarl!
		system ($connstr) == 0 or RVT_log ('ERR', "Couldn't connect resorce $n");
	}
...
sub RVT_images_scan {
  ...
  for my $images ( @{$main::RVT_cfg->{paths}[0]{images}} )  {
    ...
    while (defined(my $f=readdir(IMAGES))) {
        ...
        while (defined(my $img=readdir(CASE))) {
            my $imgpath = $images . "/$f/$img";
            next unless ($img=~/^$case-(\d\d)-(\d\d?)\.net$/ && -e $imgpath);  
            my $device = $1;
            my $disk = $2;
            
            # 1.- leer fichero
            # 2.- parsear ruta, recurso, tipo, dominio, usuario y contraseña
            
            open(NET, "<$imgpath") || RVT_log ('CRIT', "couldn't read source file");
			
			my $netCfg = eval { XMLin ($imgpath, ForceArray => 1) };

			my $n = $netCfg->{resourceName};
			if ($@ OR !( $n AND $netCfg->{type} AND $netCfg->{resource} AND $netCfg->{user} AND $netCfg->{password} )) {
				RVT_log('ERR', "Error loading $imgpath");
				next;
			}
            
            $main::RVT_cases->{case}{$case}{device}{$device}{disk}{$disk}{network} = 1;
            $main::RVT_cases->{case}{$case}{device}{$device}{disk}{$disk}{network}{$n}{type} = $netCfg->{type};			
            $main::RVT_cases->{case}{$case}{device}{$device}{disk}{$disk}{network}{$n}{resource} = $netCfg->{resource};
            $main::RVT_cases->{case}{$case}{device}{$device}{disk}{$disk}{network}{$n}{user} = $netCfg->{user};
            $main::RVT_cases->{case}{$case}{device}{$device}{disk}{$disk}{network}{$n}{password} = $netCfg->{password};       
        }
        closedir(CASE);        
