...
sub RVT_check_format ($)  {
    ...
    return 'network'		if $thing =~ /^\d{6}-\d{2}-\d{1,2}-n\w$/;
...
sub RVT_get_networknumber ($) {
	# same as before. It should be RVT_get_networkname, but I use 'number' 
	# for convinience
	# It expects this format:  case-device-disk-nNAME
	
	my $value = shift;
	my ($c, $d, $disk, $net) = split('-', $value);
    $c = RVT_get_casenumber($c);
    $d = RVT_get_devicenumber("$c-$d");
    $disk = RVT_get_disknumber("$c-$d-$disk");
    
    return 0 unless ($net =~ /^n/);
    $net =~s/n//g;
    return $net if ($c && $d && $disk && ($net =~ /^\w$/) );
}


sub RVT_split_diskname ($) {
    # takes 100ccc-DD-dd-pPP format, or 100ccc-DD-dd-nNAME, and returns an array:
    #   (100ccc,DD,dd,PP) or (100cc,DD,dd,NAME)
    # accepts '100ccc' and '100ccc-DD' and '100ccc-DD-dd' and '100ccc-DD-dd-pPP'
    # and '100ccc-DD-dd-nNAME'
    
    my $d = shift;
    my $r; 
	my $part = RVT_get_partitionnumber($d);
	my $net  = RVT_get_networknumber($d);
...
sub RVT_chop_diskname ($$) {
    ...
    return 0 unless ( $level =~ /^(case|device|disk|partition|network)$/ );
    ...
    $r .= "-n" . $n->{network} 	 if ( ($n->{network}) 	&& ($level =~ /^(network)$/));
...
sub RVT_exploit_diskname ($$) {
    ...
    my ($level, $name) = @_;
    my (@cases, @devs, @disks, @parts, @nets);
  
    return 0 unless ( $level =~ /^(case|device|disk|partition|network)$/ );
    my $type = RVT_check_format($name); 
    return 0 unless ($type =~ /^(case number|device|disk|partition|network)$/ );
    my $n = RVT_split_diskname($name);
    ...
    # exploit network level
    if ($n->{network}) {
        unshift (@nets, $name);
    } else {     
        foreach my $ii (@disks) {
            my $nn = RVT_split_diskname($ii);
            foreach my $pp ( keys %{$main::RVT_cases->{case}{$nn->{case}}{device}{$nn->{device}}{disk}{$nn->{disk}}{network}} )
                { unshift (@nets, "$ii-n$pp"); }        
        }
    }  
    return @nets if ($level eq 'network');   
...
sub RVT_fill_level {
   ...
   my ($case, $device, $disk, $partition, $network) = (
        $r->{case},
        $r->{device},
        $r->{disk},
        $r->{partition},
   ...
   # !!!!  no entiendo mi propio código  >:/
   $partition = 'p' . $partition if ($partition);

   $network = 'n' . $main::RVT_level->{network} 
                    if ( !($network) && ($main::RVT_level->{network})) ;
   
   if ($partition) {
	   $$obj = RVT_join_diskname ($case, $device, $disk, $partition);}
   
   if ($network) {
	   $$obj = RVT_join_diskname ($case, $device, $disk, $network);}
